...
def menu_files(sdk: ByseSDK):
    while True:
        print("\n=== 檔案管理 ===")
        print(
            "1. 列出檔案  2. 搜尋標題  3. 檔案詳情  4. 克隆  5. 編輯標題/公開度  6. 移動到資料夾  7. 縮圖/海報/預覽  8. 產生 Embed Code  9. 返回"
        )
            ...
            try:
                ...
                if isinstance(files, list):
                        ...
                        rows.append(
                            {
                                ...
                                "title": title[:30] if isinstance(title, str) else "",
                                "length": f.get("length"),
                                "views": f.get("views"),
                    ...
                    print_table(
                        "檔案列表",
                        rows,
                        ["file_code", "title", "length", "views", "canplay"],
                    )
                    print_json(data)
                else:
                    print_json(data)
...
def menu_upload(sdk: ByseSDK):
    while True:
        print("\n=== 上傳中心 (完整) ===")
        print(
            "1. 本地檔案上傳  2. 批次本地上傳  3. 遠端 URL 上傳  4. 批次遠端上傳  5. 查詢遠端進度  6. 移除遠端佇列  7. 返回"
        )
        c = input("選項: ").strip()
        if c == "1":
...
def menu_encoding(sdk: ByseSDK):
    while True:
        ...
        if c == "1":
            try:
                print_json(sdk.encoding_list())
            except Exception as e:
                print(f"encoding_list 失敗: {e}")
        elif c == "2":
            code = input("filecode: ").strip()
            try:
                print_json(sdk.encoding_status(code))
            except Exception as e:
                print(f"encoding_status 失敗: {e}")
        elif c == "3":
            code = input("filecode: ").strip()
            try:
                print_json(sdk.encoding_restart(code))
            except Exception as e:
                print(f"encoding_restart 失敗: {e}")
        elif c == "4":
            code = input("filecode: ").strip()
            try:
                print_json(sdk.encoding_delete(code))
            except Exception as e:
                print(f"encoding_delete 失敗: {e}")
        else:
            break
...
def menu_tools(sdk: ByseSDK):
    while True:
        print("\n=== 工具箱 ===")
        print(
            "1. 測試所有端點  2. 已刪除檔案  3. DMCA 清單  4. 產生 Premium HLS (實驗)  5. 字幕/封面/Logo URL 產生器  6. 返回"
        )
        c = input("選項: ").strip()
        if c == "1":
            res = sdk.test_all()
            ...
            try:
                print_json(sdk.deleted_files())
            except Exception as e:
                print(f"deleted_files 失敗: {e}")
        elif c == "3":
            try:
                print_json(sdk.dmca_files())
            except Exception as e:
                print(f"dmca_files 失敗: {e}")
        elif c == "4":
            code = input("filecode: ").strip()
            ip = input("觀看者 IP (如 8.8.8.8): ").strip() or "8.8.8.8"
            ua = (
                input("User-Agent (空白用預設 iPhone): ").strip()
                or "Mozilla/5.0 (iPhone; CPU iPhone OS 13_5_1 like Mac OS X)"
            )
            try:
                print_json(sdk.premium_hls(code, ip, ua))
            except Exception as e:
                print(f"premium_hls 失敗: {e}")
        elif c == "5":
            # 已在檔案管理有，但這裡再演示
            code = input("filecode: ").strip()
            ...
            url = sdk.build_embed_with_extras(
                code,
                subtitles=[{"file": sub_url, "label": sub_label}] if sub_url else None,
                poster=poster,
                logo=logo,
            )
            print(f"Embed URL: {url}")
            print(
                f"Iframe: {sdk.iframe_code(code, subtitles=[{'file': sub_url, 'label': sub_label}] if sub_url else None, poster=poster, logo=logo)}"
            )
        else:
            break
